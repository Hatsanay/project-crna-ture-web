...
endwhile;
    ...
    ,`garageprofile`,`garagelattitude`,`garagelonggitude`)
            VALUES ('$name','$name1','$name2','$name3','$graname','$granamephone','$grahousenum','$gargroup','$garalley'
      ,'$garroad','$garownerid1','$memgarid1','$_P_img','$latti','$loggi')";
    ...
    ,`ownerfname`,`ownerlname`,`ownhousenum`,`owngroup`,`ownalley`,`ownroad`,`ownersex`,`ownertel`)
            VALUES ('$ownname','$ownname1','$ownname2','$ownname3','$ownfname','$ownlname','$ownhousenum'
      ,'$owngroup','$ownalley','$ownroad','$sex','$ownphone')";
        if(mysqli_query($con,$sql_insert))
        {
            // echo '<script>alert("Product added successfully")</script>';
        }
    ...
    $memdate = date('Y-m-d');
        // $id = mysqli_real_escape_string($con,$_POST['province']);
        $sql_insert =
        "INSERT INTO `members`(`memusername`,`mempassword`,`membersemail`,`memlavel`,`memdate`)
            VALUES ('$memgarusername','$memgarpassword','$memgaremail','$memgarlevel','$memdate')";
        if(mysqli_query($con,$sql_insert))
        {
